...
% \tempend{*.sty}{*.bib}  =  temporarily ending the document with a reference listing
% \myspacing =  defined to give the correct spacing of about 23 lines per page
% \myheading = proper numbering and format
% \unit{} and \qty{}{} = SI units from siunitx package that give proper spacing between numbers and units
...
% longtable env. = for longer tables that span several pages from the longtable package - can be combined with threeparttable t
...
\section{Abstract}
\italk{It will be good to know some basics and expectations of the Grad School for NDSU disquisitions (thesis, paper, and dissertations). Of equal importance is the knowledge of \LaTeX\ the software, which has served several students, researchers, and journal/book publishers for over 40 years and continues to do so.}
...
\item \italk{The user can have all the references in one place as a \bx  ($\ast$.\vb{bib}) flat-file that can have several hundred entries, yet being ASCII, the size will be quite small. For example, for a 100-article entry, with 1757 lines of data, the size of the bib file is 68 KB, as opposed to the same content in the TPS doc file, which is 192 KB. Such master bib files will serve as the ``Once Write and Read Many'' mode of operation and can be subjected to several style formats directly. --- Usually, such automation with TPS may require additional commercial software (e.g., EndNote).} 
...
\item \italk{Advanced conditional formatting and handling of other features can be performed in \lx using the `\vb{ifthen}' \cptp package. For instance, the command} \ifthenelse{\boolean{@twocolumn}}{}{\\}\vbc{ifthenelse
...
\item \italk{Advanced use of \lx allows for drawing figures through `\vb{pstricks}' or `\vb{eepic}' packages that offer extended capabilities and produce good quality vectorized ($\ast$.\vb{eps} files) mathematical, graphical, flowchart, and geometrical figures \citep{Goossens2008g}. The method involves only codes prepared in ASCII text. --- This drawing capability is available through specific drawing tools in other TPS. Shown here is a generated picture using simple codes in `\vb{picture}' environment using \vbc{line}, and \vbc{multiput} commands, among others \citep{Kern2007a, Kern2007, Mittelbach2004}:}   
...
\italk{However, it is also possible to draw some simple pictures using `\vb{picture}' environment directly in \lx, but they are restricted in their range. Shown below are simple drawings that used \vbc{circle}, \vbc{vector}, and \vbc{framebox} commands, among others.} 
...
\item \italk{\lx can also be used to create conference posters (e.g., \vb{a0poster.cls} and \vb{sciposter.cls}) and presentation slides (e.g., \vb{beamer.cls} and \vb{prosper.cls}) using appropriate classes and packages.} 

\item \italk{Students could be able to convert their thesis into a journal article with a few easy modifications, as the basic \lx code is the same irrespective of the documentclass or template.}
...
\item \italk{Developing user-defined classes, templates, and packages will be quite complicated, as it requires knowledge of plain \tx and \lx codes. Although it should be understood that with thorough knowledge and understanding of the existing codes, it is possible to develop them as they are added on a regular basis by developers throughout the world, enriching the system. However, there is no need for a general user to venture into those areas, as most of the document preparation requirements could be addressed by employing the available sources.} 

\item \italk{Debugging the code needs some practice. As with any computer language, missing a symbol will stop the compilation with error messages or produce several errors. With experience, the users can able to decipher the error messages better and fix the code easily.}  

\item \italk{A reviewer or collaborating authors should know \lx in order to incorporate the suggestions and modify the document at the source level. The student \& advisor should be ``\lx aware'' or at least ``support'' the rendered output for the collaboration to work well.  The ``Track-changes'' feature available with TPS is not directly available with \lx but available through specific packages. However, this is not an issue for one with a working knowledge of \lx; as one can readily make annotations such as insert, delete, highlight, and comment the document with color codes and special notes (e.g., footnotes, margin notes, endnotes) as indicated earlier.} 
...
-------------[Options]-applicable in this class----------------------
 ...
 showframe = frame around the text area to check how the text fills in the 
...
---------Other useful commands or shortcuts available are:-----------
 ...
 \citep{} and \citet{} = natbib package\cptp commands for parenthetical and 
 ...
 \toprule, \midrule, \cmidrule, \bottomrule = booktabs\cptp package 
 	commands for tables
 \abovedisplayskip = to adjust the space above the displayed items, 
 ...
 \url{} = URLs break well as expected at the right margin (necessary 
 	code added in class)	
\citestyle{} = predefined natbib\cptp styles (options: plain, agu, egu, 
 ...
 \myfigls[1 optional]{5 items} = shortcut for landscape figures 
 	[caption vertical placement]{placement}{size}{file}{caption}{label}	
---------------------------------------------------------------------
 tblr env. =  automatic full-width table generation using tabularray package. 
 	Replaces tabular environment and can be used with the booktabs package
 tablenotes env. =  threeparttable package commands for tables 
 	with footnotes 
 longtable env. = for longer tables that span several pages from 
 	longtable\cptp package - can be combined with threeparttable 
---------------------------------------------------------------------
...
\italk{Equations should follow the established convention --- and loosely coding the equation and its elements is simply unprofessional. In general, such conventions, if not taken with specific care, will be overlooked, and the user feels no harm done. It was noticed that several published papers carry these mistakes --- which will not make it right, and users should not follow a bad example.}
...
\begin{tblr}{X[0.6,l] X[0.75,l] X[0.75,l] X[2.2,l]}
\toprule
\textbf{Item}		& \textbf{Correct form---Do}		& \textbf{Wrong form---\textcolor{red}{Don't}}		& \textbf{Remarks}\\
\midrule
Abbrevia\-tions & ABEN, STD, TP, TN, FP, FN & \textcolor{red}{$ABEN, STD$, $TP, TN, FP$, $FN$} & Abbreviations, usually $>1$ letter long, should always be upright. This should be followed in regular text and in equations (use the \cmd{text\{\ldots\}} * in equations or math mode). Otherwise, it may be considered as a product of variables. \\

Variables & $T, P, V, t, v$ & \textcolor{red}{T, P, V, t, v} & Variables, usually 1 letter long, should always be typeset in italics. The italics font signifies technical symbols of variables (e.g., temperature, pressure, volume, time, velocity).\\ 
...
\footnotesize
\item \hspace{-1ex} Note: * Some standard functions can be directly coded in equation or math mode with \textbackslash\: as: \cmd{sin}, \cmd{cos}, \cmd{tan}, \cmd{log}, \cmd{min}, and \cmd{max}, and so on. For others use \cmd{text\{\ldots\}}.  
\end{tablenotes}
\end{table} 
...
\italk{\LaTeX installations will have some tools (e.g., IntelliSense code completion) that allow for developing table codes from scratch or pasting the copied table data from spreadsheets (e.g., ``Paste Spreadsheet Cells -- booktabs'' in Mac), making table creation easy. Also, there are several online tools \href{https://www.tablesgenerator.com}{\textcolor{magenta}{Table Generator}} and \href{https://www.latex-tables.com}{\textcolor{magenta}{LaTeX Tables Editor}} among others will help generate table codes from typed data, and imported files (*.csv, *.xls, etc.). Also, Excel Add-Ins such as \href{https://ctan.org/tex-archive/support/excel2latex?lang=en}{\textcolor{magenta}{Excel2\LaTeX}} will generate and export the table codes. 
}

%-----------------------------------------------------------------------------
\section{Full-width Tables}
\italk{Unlike short-width tables with a few columns (\cref{tab21,tab22,tab23a}), based on the width of columns and the width exceeds about \pr{60}, it will be better to opt for full-width tables that look aligned with the surrounding text. We have two methods of achieving the full-width tables when required.}
...
\italk{The automatic method using the \vb{tblr} environment replacing the \vb{tabular} makes it simple and avoids the guesswork in fixing the table width. The following code (rest of the code is the same as Table~2.2, page: \pageref{tabcode22}) that reproduces the \cref{tab24} and the generated output (\cref{tab25}) are:} 
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \hspace{-1ex} Note: $R$, $G$, $B$ stand for red, green, blue pixel values from the RGB color image.
\label{tab27}
\end{tablenotes}
\end{table}
%------------------
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \hspace{-1ex} \emph{Note}: $R$, $G$, $B$ stand for red, green, and blue pixel values from the RGB color image.
\label{tab28}
\end{tablenotes}
\end{table}
%------------------
...
\mysectop
\section{Long Tables}
\italk{In the disquisition, sometimes it is necessary to present data and results that go beyond a single page. In such situations, long tables should be used, and the package developed for this purpose and included in the class was \vb{longtable}\cptp. The \vb{longtable} environment is used. For automatic full-width long tables, the \vb{tabularray}'s \vb{longtblr} environment is used.} 

\italk{The long tables have more components than regular tables (\cref{longtab1}). Long tables contain, in general, a main title, running title, running table head, running footnote, and table final footnote\footnote{Final elaborate longtable footnote needs special consideration. The \cmd{multicolumn\{\}} command spanning all columns and `p' format for longer text can be used---as shown in this example. Also, note that the \texttt{threeparttable} package is not compatible with \texttt{longtable}}. Users are urged to refer to the documentation of \vb{longtable} and the source code for more details, as there are several aspects involved in long table creation. Referring to the long tables can be done by defining the \texttt{label} right inside the \texttt{longtable} environment and referring to it in the usual way (\cref{longtab1} and \Cref{longtab1}).}


\subsection{Longtable 1: Elaborate Long Table}


\begingroup% required for restricting the scope commands (e.g., footnotesize, newcommand heads - local to this group) 

\newcommand{\heads}{% shortcuts possible DRY principle
\toprule
...
%-----------------------
 % multicolumn with p format can use the long caption (spacing + vspace + table name + # + title required)
\multicolumn{8}{@{}p{1\textwidth}@{}}{\vr\singlespacing\normalsize\tablename\ \thetable{}. A long table - spanning 3 pages - an example taken from our research group work on ``Methods of optimum bale stack locations and their logistics distances and methods combined distances'' --- \emph{(continued)}. } \\ %The table tile is repeated 
 ...
 %-----------------------
 % multicolumn with p format can use the long caption (spacing + vspace + table name + # + title required)
\\[-0.6in]% needed to adjust the spacing above the footnote
\multicolumn{8}{@{}p{1\textwidth}@{}}{\singlespacing\footnotesize{First line of footnote.}} \\[-0.3in]
\multicolumn{8}{@{}p{1\textwidth}@{}}{\singlespacing\footnotesize{Second line of footnote + kant [9] as \kant[9]}} 
 \endlastfoot 
...
\bottomrule
\label{longtab1} % label inside the innermost longtable environment
\end{longtable} 
}% arraystretch
}% small font
\endgroup
...
\italk{Other text continues.} \kant[9]




%-----------------------------------------------------------------------------
\subsection{Longtable 2:  Automatic Full-Width Long Table Using \texttt{xltabular} package}

\italk{With long tables that have more columns, it will always be convenient to have the automatic full-width rather than manually adjusting the column width or column separation. While \texttt{tblr} is the best option with single-page tables, the long table version of \texttt{longtblr} was found to have some issues with other setup commands---whereby the caption is not working (not displayed and appears in LOT). However, the \texttt{longtblr} can be used for other non-numbered tabular data (as presented later), since it is simpler to work with.} 

\italk{The modern and recommended solution is to use the \texttt{xltabular}\cptp package. This package is versatile and will do the following:
\begin{itemize}
\item Production of table caption and LOT entry
\item Supports all types of columns (l, c, r, p, and X)
\item Running head, foot, and final foot can be activated or suppressed---through \\ \cmd{multicolumn} command
\item X column makes the automatic width adjustment
\item The command $>$\{\cmd{raggedright}\cmd{arraybackslash}\}p\{\ldots\} makes raggedright $\quad\quad$ columns (otherwise justified)
\item No adjustment of the counter is required
\item The whole table width can be adjusted (sometimes necessary when the final column has citations)---comes handy with landscape environment (1.346 \cmd{textwidth}; See: \Cref{tab:xltabularls})
\item Can be used for all practical long table situations---just follow the subsequent elaborate example shown in \Cref{tab:xltabular}.
\end{itemize}
}

%-----------xltabular---------------
\begingroup% required for restricting the scope commands (e.g., footnotesize, newcommand tf - local to this group) 

\newcommand{\tf}{\textcolor{magenta}{Auto full-width X column:} It remains a mystery why, then, the discipline of human reason, in other words, is what first gives rise to nothing!}
\footnotesize
\renewcommand{\arraystretch}{2}% adjusting the rowspacing
\begin{spacing}{0.6}


\begin{xltabular}{\textwidth}{l X c >{\raggedright\arraybackslash}p{0.6in} r}% at least one X column required
\caption{Longtable using xltabular package illustrating the regular \texttt{l, c, r, p} and automatic \texttt{X} columns.}       % <---
\label{tab:xltabular}\\

%%%  (1) first head
   \toprule
First l column & \textbf{Second X column} & \textbf{Third c column} & Where? p & Number r\\
    \midrule
    \endfirsthead

%%%  (2) running head
    \multicolumn{5}{l}{\!Table running header---\textcolor{blue}{comment this when not required} or \hl{repeate the first head - NDSU recommendation}}\\  % If required, otherwise commented
    \toprule
First l column & \textbf{Second X column} & \textbf{Third c column} & Where? p & Number r\\
    \midrule
    \endhead
...
%%%  (4) last foot
\\[-0.5in]% needed to adjust the spacing above the footnote
\multicolumn{5}{@{}p{6.5in}@{}}{\singlespacing\scriptsize{First line of footnote.}} \\[-0.35in]
\multicolumn{5}{@{}p{6.5in}@{}}{\singlespacing\scriptsize{Second line of footnote + kant [9] as \kant[9]}} 
\endlastfoot

% Table data
One & \tf & 123.456778  & Go go go go \ldots & \num{71294539}\\
One & \tf & 123.456778  & Go go go go \ldots & \num{71294539}\\
One & \tf & 123.456778  & Go go go go \ldots & \num{71294539}\\
One & \tf & 123.456778  & Go go go go \ldots & \num{71294539}\\
One & \tf & 123.456778  & Go go go go \ldots & \num{71294539}\\
...
\bottomrule
\end{xltabular}
\end{spacing}% adjusting line spacing in paragraphs
\endgroup
...
%-----------------------------------------------------------------------------
\subsection{Longtable 3: Landscape Automatic Full-Width Long Table Using \texttt{xltabular} package}

\italk{Wrapping the long table \texttt{xltabular} environment with \texttt{landscape} environment readily creates the landscape longtable (\cref{tab:xltabularls}). The width of the table should be adjusted to cover the entire width (e.g., 1.346\cmd{textwidth}). Note that the textwidth corresponds to the regular upright page. Footnote width should also be adjusted.}
...
\newcommand{\heads}{% headings shortcuts made 
\toprule
First l column & \textbf{Second X column} & \textbf{Third c column} & Where? p & Number r\\
    \midrule
}
...
\begin{xltabular}{1.346\textwidth}{l X c >{\raggedright\arraybackslash}p{0.6in} r}
\caption{Landscape longtable using xltabular package illustrating the regular \texttt{l, c, r, p} and automatic \texttt{X} columns with no repeating head.}             % <---
\label{tab:xltabularls}\\
...
%%% (4)  last foot
\\[-0.6in]% needed to adjust the spacing above the footnote
\multicolumn{5}{@{}p{1.346\textwidth}@{}}{\singlespacing\scriptsize{First line of footnote.}} \\[-0.45in]
\multicolumn{5}{@{}p{1.346\textwidth}@{}}{\singlespacing\scriptsize{Second line of footnote + kant [9] as \kant[9]}} 
\endlastfoot
...
%-----------------------------------------------------------------------------
\subsection{Longtable 4: Simplified Manual Full-Width Long Table With Paragraph Columns---No Repeated Caption and Header}
...
%-----------------------------------------------------------------------------
\subsection{Longtable 5: Simplified Long Table --- No Repeated Caption and Header}
...
%-----------------------------------------------------------------------------
%\vc{-0.4}
\subsection{Longtable 6: Simplified Long Table---With Header But No Repeated Caption --- No Full-width---Using \texttt{l} and \texttt{r} specifiers}
...
% For footnotes - p value by trial and error
\\[-0.4in]
\multicolumn{5}{l}{\footnotesize My footnote for the table is coded here. Longer note below.}\\[-0.9cm]
\multicolumn{5}{p{6in}}{\singlespacing\footnotesize \kant[9]}
\endlastfoot
...
%-----------------------------------------------------------------------------
\mysectop
\subsection{Longtable 7: Simplified xltabular --- With Header But No Repeated Caption}

\italk{Since it is an automatic full-width (specified width and one X column), no centering is necessary. Row spacing by \texttt{spacing} command (others are ineffective with the class). The necessary codes (caption, footnote, column specifications, etc.) will be as usual (see previous examples). Enclosing the group environment is necessary. As this is an un-numbered table, the table's serial number should be reduced by 1 using the command \cmd{addtocounter}\{table\}\{-1\} after the table code (as applied at the end).}
...
% use >{}X format for alignment and >{}p{width} for alignment and width
\begin{xltabular}{\textwidth}{l >{\raggedright\arraybackslash}p{0.3\textwidth} X l  >{\raggedleft\arraybackslash}X @{}} 
...
% For footnotes - p value by trial and error
\\[-0.4in]% needed to adjust the spacing above the footnote
\multicolumn{5}{p{\textwidth}}{\singlespacing\footnotesize \emph{Note}: \kant[9]}
\endlastfoot
...
%-----------------------------------------------------------------------------
\vs{-0.02}
\subsection{Longtable 8: Simplified Longtable --- With No Header or Repeated Caption - Used to Present Just Long Data}
...
%-----------------------------------------------------------------------------
\subsection{Longtable 9: Simplified Longtblr --- With No Header or Repeated Caption - Used to Present Just Long Data}
...
%-----------------------------------------------------------------------------
\subsection{Longtable 10: Simple Long Data Table Using \texttt{tabbing} Environment}\label{lttab}
...
%-----------------------------------------------------------------------------
\subsection{Longtable 12: Landscape Long Tables Using \texttt{longtable}}
\italk{Applying the similar logic \texttt{longtable} when enclosed in \texttt{landscape} environment will produce the landscape long tables (\cref{longtabls}). The previous table was reproduced to demonstrate long tables in landscape format. Note the use of \cmd{setlength}\cmd{tabcolsep}\{5.25pt\}, obtained through trial-and-error, to adjust the final width of the table by altering the column separation spacing.}
...
\begin{landscape}
{\small 
\setlength{\tabcolsep}{5.25pt}% used to control the final width of the table
{\renewcommand{\arraystretch}{0.6}
...
{{\normalsize\tablename\ \thetable{}. Methods of optimum bale stack locations and their logistics distances  -- (\emph{continued}).}} \\[2ex]
 \tabletopinfols% defined in the preamble
    ...
    \cmidrule{10-11}% running foot
    \multicolumn{11}{r}{\textit{continued \ldots}}
    \endfoot
    
   
    \bottomrule% final footnote
\\[-0.4in]% needed to adjust the spacing above the footnote
\multicolumn{11}{@{}p{1.345\textwidth}@{}}{\singlespacing\footnotesize{First line of footnote.}} \\[-0.3in]
\multicolumn{11}{@{}p{1.345\textwidth}@{}}{\singlespacing\footnotesize{Second line of footnote + kant [9] as \kant[9]}} 
...
\end{center}
\caption[Title of my figure which is displayed in the list of figures - details given separately in this long caption.]{Title of my figure which is displayed in the list of figures - details given separately in this long caption. \textcolor{magenta}{Long caption not shown in TOC - but the contents are added as regular text in the figure environment --- as shown here}. Details of the item shown in the figure are: (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here in a new line a long description about the figure, (b) Here in a new line a long description about the figure, (c) Here in a new line a long description about the figure, (d) Here in a new line a long description about the figure. (a) Here, in a new line, a long description about the figure.}
...
\subfloat[SubCap] {\includegraphics[width=0.2\textwidth]{frog.jpg}}\\
\captionsetup{singlelinecheck=false} % can be given again
\caption[]{Multiple page sub-figures --- This caption can be the same as above or abbreviated. \textcolor{magenta}{Notice that the figure number was dropped in the numbering}. This is the final caption.} 
\end{figure}
...
\italk{We have used \cmd{cref\{\ldots\}} commands already in the previous chapters. The \vb{cleveref} package documentation may be referred to for other commands and options. The package allows for referring ranges, multiple items, page numbers, and many more customizations.  
}
...
\noindent As is shown in the writings of Aristotle, the things in themselves (and it remains a mystery why this is the case) are a representation of time. Have alone been able to show that.
...
Our sense perceptions constitute a body of demonstrated doctrine, and some of this body must be known a posteriori. Human reason occupies part of the sphere of our experience concerning the existence of phenomena in general. Things in themselves (and it remains a mystery why this is the case) of time. \italk{Now, \cmd{mygather\{\ldots\}} shortcut:}
...
\noindent \italk{produces a hyperlink (shown in magenta subsequently) that points $\Rightarrow$ }\textcolor{magenta}{\url{https://www.pearson.com/us/higher-education/program/Lamport-La-Te-X-A-Document-Preparation-System-2nd-Edition/PGM159713.html}} \italk{to the webpage. Also, notice how the URL was correctly broken to fit the margin, and hovering on the URL will show the complete working URL when clicked, which will take the user to the webpage.}
...
\centering
\caption{One appendix full-width table caption goes at the top of the table.}
...
\italk{We know well that long tables are a little involved and tricky, and then in the appendix, need manual override for proper output. The regular (in chapters) \vb{longtable}, \vb{xltabular}, and \vb{longtbrl} environments are used to code the long tables, and their captions, and LOAT entries will appear correctly with \vb{table} environment. However, the existing appendix \vb{appendixtable} environment will not support the \vb{longtable}, \vb{xltabular}, and \vb{longtbrl} environments, even though these will output table contents, the caption table numbering, and LOAT will not appear correctly. Therefore, a simpler fix followed includes (1) a dummy \vb{appendixtable} with only a caption for the long table with negative \vb{vspace} and optional TOC entry \cmd{caption}[\ldots] with repeated caption text without negative \vb{vspace}, and immediately followed by (2) the long table code without caption.} 

\italk{The shortcut (\cmd{mytabapcap} with 1 optional [for vertical adjustment; lifting the table up; defaulty unit = $-8$ex] + 2 mandatory arguments \{Long table main caption\} and \{label\}) can be conveniently used to create the caption. This code will take care of the correct numbering, and the regular caption in the long table should be suppressed. Examples of the appendix long tables are shown in Table~\ref{aplongtab1}, Table~\ref{aplongtab2}, and Table~\ref{aplongtab3}.} 

\italk{As shown before (Sec.~\ref{lttab}), for simple long data, the \cmd{tabbing} environment can be utilized. These methods of the appendix long table can be coded with or without a caption. These advanced tables should be referenced using the regular \cmd{ref}\{\ldots\} commands (e.g., Table~\ref{aplongtab1} and Table~\ref{aplongtab2}).}

\textcolor{magenta}{In summary, we have \cmd{mytabapcap} with \vb{longtable}, \vb{xltabular}, and \vb{longtbrl} environments to code appendix long tables. Recall for regular chapters, we use \vb{longtable} or \vb{xltabular}, otherwise the workaround combination of \cmd{mytabcap} and \vb{longtbrl}.}  
...
%-----------------------------------------------------------------------------
\vspace{-0.01in}
\subsection{Appendix Long Table Using \texttt{xltabular}}
\kant[9]

%-----------xltabular---------------
\mytabapcap[-7ex]{Appendix longtable using xltabular package illustrating the regular \texttt{l, c, r, p} and automatic \texttt{X} columns.}{aplongtab3}
...
\begin{xltabular}{\textwidth}{l X c >{\raggedright\arraybackslash}p{0.6in} r}
%\caption{Longtable using xltabular package illustrating the regular \texttt{l, c, r, p} and automatic \texttt{X} columns.}             % <---
%\label{tab:xltabular}\\
   \toprule
First l column & \textbf{Second X column} & \textbf{Third c column} & Where? p & Number r\\
...
%%%  running head
    \multicolumn{5}{l}{\!Table running header---\textcolor{blue}{comment this when not required}}\\  % If required, otherwise commented
    \toprule
First l column & \textbf{Second X column} & \textbf{Third c column} & Where? p & Number r\\
    \midrule
    \endhead
...
%%%  last foot
\\[-0.5in]% needed to adjust the spacing above the footnote
\multicolumn{5}{@{}p{6.5in}@{}}{\singlespacing\scriptsize{Note: First line of note.}} \\[-0.35in]
\multicolumn{5}{@{}p{6.5in}@{}}{\singlespacing\scriptsize{Note: Second line + kant [9] as \kant[9]}} 
\endlastfoot
...
\italk{The actual example or rendered section of a Java program using \vb{lstlisting} environment (refer to source code) is shown below as an illustration:}
...
\vfill
{\noindent\scriptsize\textcolor{gray}{Updated: March 22, 2026}}
