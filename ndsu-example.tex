%******************* START *** [Updated: 14-03-2026] *******************
\documentclass[ms-thesis,mathdesign,12pt,showgrid]{ndsu-thesis-2022}

%Refer to documentation (ndsu-thesis-2022-documentation....) for various options and commands

\graphicspath{{./figures/}}
%\renewcommand\myspacing{1.9} % 23 lines/page needs 1.9; other values for other fonts
...
\title{The Title of My M.S. Thesis: Optimized for two-line titles and the vertical starting level can be adjusted}
\author{Samuel Fargo Bison}
\pdftitleauthor{The Title of My M.S. Thesis: Optimized for two-line titles and the vertical starting level can be adjusted}{Samuel Fargo Bison}% For PDF output - title and author repeated
\date{December 2026}
...
\mypaperheading{The First Chapter - Paper Style - Long title of this technical paper}{ Footnote only for published paper. Include citation for the peer-reviewed article + more information about the authors' contributions. Refer to NDSU dissertation resources for examples.}
...
\vo
\myfig[0.15in]{h!}{0.525}{example-image-duck}{Caption for this example image in this first chapter.}{fig:1}	
...
\item \hspace{-1ex}The \texttt{tablenotes} environment produces table footnotes. 
\end{tablenotes}
\label{tab:2}
\end{table}	
...
\kant[8]
The \Cref{fig:2} is an example image with a command showing all arguments, including the optional caption placement. The example figure (\cref{fig:2}) is included in the \texttt{mwe} package.

\vo
\myfig[1.5ex]{h!}{0.32}{frog}{Caption for this example image demonstrating an optional 2ex vertical spacing. Compare this with a narrow caption spacing without an optional argument in 
\cref{fig:1}.}{fig:2}
...
The regular way of coding a scheme (\cref{sc1}; \hl{Observe vertical spacing in source code; case of float between two paragraphs; shortcuts around the figure environment}): 
...
Check this! \hl{Observe vertical spacing in source code; case of float embedded in a paragraph; paragraph text vertical spacing adjusted by 7th argument} of \texttt{myschs} command): \kant[10]

\myfigs[0.1in]{t!}{0.7}{frog}{Caption for this example image demonstrating an optional 0.1in vertical spacing. Compare this with a narrow caption spacing without an optional argument in \cref{fig:2}.}{sc2}{-0.1in}
...
\vo
\myfigap[0.15in]{h!}{0.5}{example-image-golden}{A golden ratio rectangle image.}{fig:ap1}	
...
\begin{appendixtable}[h]
\centering
\caption{Squares and cubes in named appendix table using \texttt{siunitx} and \texttt{tabularray} packages.}
\vh
