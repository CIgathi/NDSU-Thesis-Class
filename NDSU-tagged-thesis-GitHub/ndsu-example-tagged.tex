...
%***********************************************
%******************* START *******************
\documentclass[ms-thesis,mathdesign]{ndsu-thesis-tagged}% MS final, 12 pt
%\documentclass[ms-thesis,mathdesign,chaprefs]{ndsu-thesis-tagged}% MS final, chap refs, 12 pt
%\documentclass[ms-thesis,mathdesign,showgrid]{ndsu-thesis-tagged}% MS work, 12 pt
%\documentclass{ndsu-thesis-tagged}% PhD final, 12 pt, computer modern
%\documentclass[mathdesign,chapterrefs,showgrid]{ndsu-thesis-tagged}% PhD, chap refs, work
...
\acknowledgements{\start I acknowledge people here. \\ \tcb{\emph{Acknowledgements text should be placed here.}} \\ \indent\repeatN{100} \\ \indent\repeatN{80}
}%
...
\indent\repeatN{100} \\ \indent\repeatN{80}
}%
...
%------------------------------------------------------
\section{Abstract}
Paper-styled chapters will have abstracts (PDF tag \tcb{H2}). \tcb{This \LaTeX{} \texttt{ndsu-thesis-tagged} class will automatically tag all elements (H1--H8, paragraph, caption, TH, TD, etc.) of the thesis (when instructions are followed properly)}. Abstract of this chapter goes here. Have your abstract text here. \repeatN{13}

%------------------------------------------------------
\section{Section ($\Rightarrow$ 1st level; Title Case; Centered; Boldface) E.g., Science is Key Period}
This is the first section of the thesis (1st level: 1.2. Section, PDF tag \tcb{H2}). \repeatN{22}

%------------------------------------------------------
\section{Section E.g., Science is Key Period}
This is the second section of the thesis (1st level: 1.3. Section, PDF tag \tcb{H2}). \repeatN{21}

%------------------------------------------------------
\subsection{Subsection ($\Rightarrow$ 2nd level; Title Case; Left-justified; Boldface) E.g., Science is Key Period}
This is the subsection text (2nd level: 1.3.1. Subsection, PDF tag \tcb{H3}). \repeatN{22}

%------------------------------------------------------
\subsubsection{Subsubsection ($\Rightarrow$ 3rd level; Sentence case; Left-justified; Boldface; Italics) E.g., Science is key period}
This is the subsubsection text (3rd level: 1.3.1.1. Subsubsection, PDF tag \tcb{H4}). \repeatN{82}

%------------------------------------------------------
\paragraph{Paragraph ($\Rightarrow$ 4th level; Sentence case; Left-justified; No bold; Italics)  E.g., Science is key period}
This is the paragraph text (4th level: 1.3.1.1.1. Paragraph, PDF tag \tcb{H6}). \repeatN{21}

%------------------------------------------------------
\subparagraph{Subparagraph ($\Rightarrow$ 5th level; Sentence case; Left-justified; No bold; Regular) E.g., Science is key period}
This is the subparagraph text (5th level: 1.3.1.1.1.1. Paragraph, PDF tag \tcb{H7}). \repeatN{20}
...
\vh% shortcut = \vspace{-0.05in}
\myfigst[-0.05in]{ht!}{0.6}{example-image-duck}{Caption for this example image in this first chapter using the tagged shortcut command \texttt{myfigst} that pulls the embedded text up.}{A yellow duck with a golden crown and pizza in the belly on a gray background.}{fig:11}{-0.1in}	
...
\caption{Table spanning full-width using \texttt{xltabular} environment---autmatic method.} \label{tab:22}\\[-0.15in]
\toprule
Number 	& Name of month 	& Days 	& Season\\
\midrule
\#4 		& April  			& 30		& Spring\\
...
\voh
\repeatTN{filler text}{55}

%------------------------------------------------------
\subsection{Minor Results}
This is a subsection of the thesis (1st level: 2.2. Experimental Results). \repeatN{42}
...
\vo
\myfigt[-0.05in]{h!}{0.365}{frog}{Caption for this example image demonstrating an optional \qty{0.025}{in} vertical spacing. Compare this with a narrow caption spacing without an optional argument in \Cref{fig:11}.}{A green frog with a yellow chest that is ready to hop.}{fig:21}

\vrh% shortcut = \vspace{-0.35in}
\repeatTN{my filler text}{38}
...
%------------------------------------------------------
\section{Schemes}
The shortcut was used to code the scheme (\cref{sch:21}), which is a substitute for the regular way using \texttt{includegraphics} command. \hl{Observe vertical spacing in source code; case of float between two paragraphs; shortcuts around the figure environment}: 

\repeatTN{my dummy text}{48}

\vc{-0.6}% shortcut:  vc = \clearpage\vspace*{#1in}
\myschst[-0.1in]{h!}{0.5}{LampFlowchart}{Flowchart of controls of light bulb---A scheme---using myschst command.}{A flow diagram showing the steps of troubleshooting a lamp not working.}{sch:21}{-0.15in}

Check this! \hl{Observe vertical spacing in source code; case of float embedded in a paragraph; paragraph text vertical spacing adjusted by 7th argument} of \texttt{myschst} command). 

\myfigst[-0.05in]{t!}{0.7}{frog}{Caption for this example image demonstrating an optional -0.08in vertical spacing. Compare this with a narrow caption spacing without an optional argument in \Cref{sch:21}.}{A large-sized green frog with a yellow chest that is ready to hop.}{fig:22}{-0.1in}

\voh
\repeatTN{filler text}{150} 
...
\myschlst[-0.2in]{p}{0.65}{LampFlowchart}{Landscape scheme---Flowchart of controls of light bulb. Optional -0.2in vertical spacing was used. Landscape can be applied to tables, figures, and schemes; also, these can be coded in appendices.}{A flow diagram showing the steps of troubleshooting a lamp not working as a landscape page.}{sch:22}

\repeatN{58}

%------------------------------------------------------
\section{Source Code Listing Inside Regular Chapters}
\tcb{Listings of source codes are complicated. The excellent ``\texttt{listing}'' package is not compatible with tagging; therefore, we use a simpler yet versatile ``\texttt{fancyverb}'' package and developed the ``mylisting'' environment, applicable to both regular chapters and appendices. The strategy consists of two steps. (1) Code the caption with \cmd{myliscap} or \cmd{mylisapcap}, and (2) Code the actual listings with ``\texttt{mylisting}'' environment with their options [fontsize, frame, numbers, xleftmargin] coded with default. Follow the code below and in the appendix.}

\vo
% Step 1 of 2
\myliscap[-0.3in]{Python code listing for finding whether a given number is prime.}{lis:21}

% Step 2 of 2
% Other options [font sizes] [frame=none, single] [numbers=none, left, right] [xleftmargin]
\begin{mylisting}[fontsize=\small, frame=single, numbers=left]
import math
...
\tcb{Equations should follow the established convention --- and loosely coding the equation and its elements is simply unprofessional. In general, such conventions, if not taken with specific care, will be overlooked, and the user feels no harm done. It was noticed that several published papers carry these mistakes---which will not make it right, and users should not follow a bad example.}
...
\toprule
\textbf{Item}		& \textbf{Correct form---Do}		& \textbf{Wrong form---\textcolor{red!80!black}{Don't}}		& \textbf{Remarks}\\
\midrule
Abbrevia\-tions & ABEN, STD, TP, TN, FP, FN & \textcolor{red!80!black}{$ABEN, STD$, $TP, TN, FP$, $FN$} & Abbreviations, usually $>1$ letter long, should always be upright. This should be followed in regular text and in equations (use the \cmd{text\{\ldots\}} * in equations or math mode). Otherwise, it may be considered as a product of variables. \\

\cmidrule(lr){2-4}
Variables & $T, P, V, t, v$ & \textcolor{red!80!black}{T, P, V, t, v} & Variables, usually 1 letter long, should always be typeset in italics. The italics font signifies technical symbols of variables (e.g., temperature, pressure, volume, time, velocity).\\ 

\cmidrule(lr){2-4}
Subscripts and superscripts & $T_\text{avg}$, $\text{TP}_{\max}$, $\text{RMSE}_{\text{observed}}$, $v^\text{in}$, $t^\text{output}$, $P^\text{top}_\text{min}$ 
& \textcolor{red!80!black}{$T_{avg}$, $\text{TP}_{max}$, $\text{RMSE}_{observed}$, $v^{in}$, $t^{output}$, $P^{top}_{min}$ } 
...
& \textcolor{red!80!black}{i, j, k, l, m,  $x_\text{i}, y_\text{j}, z_\textit{1}, \theta_\textit{23}$} 
...
& \textcolor{red!80!black}{$abs, sin, cos,$ $min, max$} 
...
% 4 of 4
    \bottomrule
    \multicolumn{4}{@{} p{\textwidth} @{}}{\vspace{-0.35in} \singlespacing The final footnote here. \repeatN{50}} \\
    \endlastfoot
...
\vo
\myfigapt[-0.05in]{h!}{0.5}{example-image-golden}{A golden ratio rectangle image from the \texttt{mwe} package.}{A gray rectangle drawn with the golden ratio.}{fig:a1}	
...
\tcb{See Listing~\ref{lis:21} preamble for information.} \repeatN{68}

\vo
% Step 1 of 2
\mylisapcap[-0.25in]{Java code listing for finding all prime numbers from 1 to 100.}{lisa:1}

% Step 2 of 2
% Other options [font sizes] [frame=none, single] [numbers=none, left, right] [xleftmargin]
\begin{mylisting}[fontsize=\normalsize, frame=single, numbers=none, xleftmargin=0pt]
...
\tcb{Appendix material can be included here. First, including a figure (fig.~\ref{fig:ap2}).}
...
\vh
\myfigapt[-0.05in]{h!}{0.8}{utbmciadreport-ciad-frontbackground}{A stock picture from the TeXLive 2025 texmf-dist distribution from UTBM-CIAD report.}{A stock image from the TeXLive 2025 texmf-dist distribution from UTBM-CIAD showing a human and robotic hand meeting.}{fig:ap2}
...
\repeatTN{my repeated sample filler text}{30}
