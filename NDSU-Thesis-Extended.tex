...
For IEEE (numercal style journals) use the following in the preamble:
...
\italk{The IEEE that requires a numerical style of citation. The above command with \texttt{style=1eee} work well with \texttt{natbib=true} option. With this the regular natbib commands like \cmd{citep\{\}} and \cmd{citet\{\}} will produce the expected output in numerical style. These commands produce correct outputs with numbered citations and author with number citations, respectively. It should be noted that the simple/regular \cmd{cite\{\}} command will only produce the number but not the author when needed.}
...
\item
\italk{With tables (and with figures on lesser extent) the placement specifier [\texttt{H}], though puts the table ``HERE'' but introduces additional row spacing. Therefore, the solution is to just use [\texttt{h!}] that keeps the rows intact.}
...
{\onehalfspacing
\begin{verbatim}
\myfigap[1.5ex]{h!}{0.45}{frog.jpg}{Appendix one - figure using myfigap 
