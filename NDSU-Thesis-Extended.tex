...
%-----------------------------------------------------------------------------
%-----------------------------------------------------------------------------
\mypaperheading{General aspects---Paper-Styled Chapter---some study titles are long, and we are making it long enough so that it flows more than two lines - oops it went to the fourth}{\hl{Update: Only published papers should have the footnote and use the \texttt{mypaperheading}---so most should use \texttt{myheading} command.} This paper is planned to be submitted as a review article in the \emph{Advanced Technical Research Collection} journal.  All the co-authors have assisted in the research direction and review of the manuscript.}
...
\singlespacing
\raggedright
\textit{``\LaTeX\ is easy to use---if you're one of the \pr{2} of the population who thinks logically and can read an instruction manual. The other \pt{98} of the population would find it very hard or impossible to use.''} 
...
The loading and unloading time can be determined using the number of bales generated and the loading and unloading time (min) per bale input as follows:
...
\noindent where, $T_\te{LU}$ is total bale loading and unloading time (h), NB is the number of bales, $T_\te{L}$ is the loading time per bale (min), $T_\te{U}$ is the unloading time per bale (min), $T_\te{OT}$ is the overall bale movement time (h), and $T_\te{C}$ is the bale collection time (min). 
...
%\newpage
\mytabcap[-7.2ex]{Full-width longtable using longtblr with caption workaround using a dummy caption through \cmd{mytabcap} command.}{tab:lt4}
\begingroup
...
note{} = {\footnotesize 
  ...
  colspec = {X X[1.5] X[1.1] X[1.2,c] X[0.8,r]},% the usual column specificification here
  rowhead = 1,% one row (the top header row) was repeated; 0 makes no repetition
}
%\caption{Summa thaan}
\toprule
...
\begin{landscape}
\mytabcap[-7.2ex]{Landscape full-width longtable using longtblr with caption workaround using a dummy caption through \cmd{mytabcap} command.}{tab:landtblr}
\begingroup
\begin{spacing}{0.84}% works with longtblr
\DefTblrTemplate{contfoot-text}{default}{\itshape Continued \ldots}% defining default 
\DefTblrTemplate{capcont}{default}{}% suppress the repeating head caption
...
\renewcommand*{\thesubfigure}{\thefigure.\arabic{subfigure}}%with fig.No.
\end{verbatim}
...
{\fontfamily{put}\selectfont\LARGE F\@., O\@., T\@., P\@.; V\@.; W\@.;} \italk{and} {\fontfamily{put}\selectfont\LARGE Y\@.}
...
%\newpage
\mytabcap[-7.5ex]{Full-width long table using \texttt{longtblr} environment with separate caption and long table code using \textbackslash\texttt{mytabcap} command.}{aplongtab2}
{%long table code
...
\newpage
\mytabapcap[-7.2ex]{Full-width appendix long table using \texttt{longtblr} environment with separate caption and long table code.}{aplongtab2}
{%long table code
\begin{spacing}{0.8}% works with longtblr
\DefTblrTemplate{contfoot-text}{default}{\itshape Continued \ldots}% defining default 
\DefTblrTemplate{capcont}{default}{}% suppress the repeating head caption
...
\lstset{
    ...
    xleftmargin=0.0\textwidth,  % Add left margin to avoid overflow
    xrightmargin=0.0\textwidth  % Add right margin to avoid overflow
}

\vspace{-3ex}
\section*{Listing of image processing and model development and evaluation in Python}
% Switch to one-column mode for the code
\vspace{1ex}
{\onehalfspacing
...
\italk{We know well that long tables are a little involved and tricky, and then in appendix needs manual override for proper output. The regular \vb{longtable} and \vb{longtbrl} environments are used to code the long tables, and their captions, and LOAT entries will appear correctly with \vb{table} environment. However, the existing appendix \vb{appendixtable} environment will not support the and \vb{longtable} and \vb{longtbrl} environments, even though these will output table contents the caption table numbering and LOAT will not appear correctly. Therefore, a simpler fix followed includes (1) a dummy \vb{appendixtable} with only a caption for the long table with negative \vb{vspace} and optional TOC entry \cmd{caption}[\ldots] with repeated caption text without negative \vb{vspace}, and immediately followed by (2) the long table code without caption. Examples of the appendix long tables are shown in Table~\ref{aplongtab1} and Table~\ref{aplongtab2}. As shown before (Sec.~\ref{lttab}), for simple long data the \cmd{tabbing} environment can be utilized. These methods of appendix long table can be coded with or without a caption.}  
