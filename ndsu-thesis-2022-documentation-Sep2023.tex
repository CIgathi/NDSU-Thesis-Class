...
\mypaperheading{The First Chapter - Paper Style - Long title of this technical paper}{$^1$}
...
\noindent where $y$ is the dependent variable, $m$ is the slope, $x$ is the independent variable, $c$ is the $y$ intercept, 
NCF is the normalized conversion factor, $S_\text{factor}$ is the scale factor, $c_p$ is the specific heat capacity at 
constant pressure ($p$, variable), and $M_\text{p}$ is the mass of a proton (p, descriptive). Note how variables, abbreviations, 
and subscripts are coded in \cref{eq:lin}. Refer Extended Thesis to know more about equations and shortcuts. 
...
Each \cmd{myheading\{\ldots\}} or \cmd{mypaperheading\{3 args\}} command starts a new page and entry in the table of contents.  The regular chapter command is simple and takes one argument which is the title of the chapter as \cmd{myheading\{title\}}. The paper-styled chapter takes three arguments that address the title and footnote as \cmd{mypaperheading\{title\}\{\ix{footnotemark}\}\{\ix{footnotetext}\}}.  The \texttt{footnotemark} should be numbers that reflect the sequential numbering of the footnotes in the document. The \texttt{title} is common to both styles and will be rendered as all-caps irrespective of the input.  
...
New commands such as \cmd{\ix{checkBeginRefsection}} and \cmd{\ix{checkEndRefsection}} can be considered as an environment that immediately follows each chapter title and encloses the chapter contents (similar to \texttt{\ix{refsection}} environment). At the end, usually before appendices, the \cmd{\ix{checkMakeCombinedReferences} command was once issued. 
...
The ASABE style journal references can be reproduced using \cmd{usepackage[style=apa,} \texttt{natbib=true]} \texttt{\{biblatex\}}. For \texttt{IEEEtran} styles with \blt use \cmd{usepackage[style=ieee]\{biblatex\}}. With \bt \texttt{*.bst} files and \cmd{citestyle\{\ldots\}} commands are available to create the style of reference followed in various departments (see Sec.~\ref{refhandling} for details). 
 ...
 %were similar to  \texttt{apalike2} style but they use \& before the last author instead of ``and''. Therefore a modified style that achieves this called \texttt{myASABE.bst} was developed and included to produce the desired outputs (however, \blt with \texttt{style=apa} produces the \& directly). For other commands, the \texttt{natbib} package documentation should be referred to. For \texttt{IEEEtran} family of styles (\texttt{-N, -S, -SA, -SN}) that are numeric bibliography styles, use option ``plain'' in ``citestyle'' command as \cmd{citestyle\{plain\}}. It is also possible to load the package with the options directly as well in one command. With \blt use \cmd{usepackage[style=ieee]{biblatex}}. 
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \hspace{-1ex} \emph{Note}: wh. refs. - whole document single reference chapter; cht. refs. - chapters individual reference sections. 
\end{tablenotes}
\label{tends}
\end{table}
